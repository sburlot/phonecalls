...
- (void) fetchAllAddressBookRecords
{
    ...
    for ( int i = 0 ; i < n ; i++ ) {
        ...
        if ((firstName == NULL) && (lastName == NULL)) {
            ...
            displayName = [[displayName replace:RX(@"\\s+$") with:@""] mutableCopy];
        }
...
//        NSLog(@"Name %@ %@ (%@) => %@", firstName, lastName, companyName, displayName);
        ...
        for (CFIndex j = 0; j < ABMultiValueGetCount(phones); j++) {
            ...
            if (phoneNumberRef != NULL) {
                ...
                phoneNumber = [phoneNumber replace:RX(@"\\s+|\\(|\\)|-|\\/|\\.") with:@""];
                phoneNumber = [phoneNumber replace:RX(@"^021") with:@"+4121"];
                phoneNumber = [phoneNumber replace:RX(@"^022") with:@"+4122"];
                phoneNumber = [phoneNumber replace:RX(@"^079") with:@"+4179"];
                phoneNumber = [phoneNumber replace:RX(@"^076") with:@"+4176"];
                phoneNumber = [phoneNumber replace:RX(@"^077") with:@"+4177"];
                phoneNumber = [phoneNumber replace:RX(@"^00") with:@"+"];
...
//                NSLog(@"%@ (%@) => %@", displayName, phoneLabel1, phoneNumber);
                [tempDictionary setObject:[NSString stringWithFormat:@"%@ (%@)", displayName, phoneLabel1] forKey:phoneNumber];
            }
        }
    }
...
- (NSArray *) checkPhoneList:(NSArray *)phoneListArray
{
    ...
    [phoneListArray enumerateObjectsUsingBlock:^(NSDictionary *dict, NSUInteger idx, BOOL *stop) {
        ...
        if (person != NULL) {
            [newDict setObject:person forKey:@"name"];
        } else {
            [newDict setObject:@"Inconnu" forKey:@"name"];
        }
...
- (NSDictionary *) checkPhoneCalls:(NSDictionary *)phoneCalls
{
    ...
    [phoneCalls enumerateKeysAndObjectsUsingBlock:^(id key, NSArray *phoneArray, BOOL *stop) {
        ...
        [responseDict setObject:responseArray forKey:key];
    }];
    return responseDict;
}
